...
if (!$SpeedtestHealth) {
    $SpeedtestHealth = "Connection speed is within 20% of last results. Below are current test results."
}
$SpeedtestHealth
$SpeedtestResults

if ($PreviousResults) {
write-host "Below are also the last results (not the above run) of the SpeedTest that was run on the device"
$PreviousResults
}
